...
%10.11 note:
%Scott's code, except doesn't add chars if they're not numbers, if string
%length == 3, and checks to see if the number is between 0 and 100.
    ...
    switch(abs(mychar))
        case {13,3, 10} % <return> or <enter>
            if acceptblanks || ~isempty(string)
                if str2double(string) <= 100 %added to JOL version
                    endRT = GetSecs-t1;
                    break;
                end
            end
...
%           end
          ...
          if sum(ismember(numbers, mychar))
              if length(string) < 3 %don't print
                  norm = Screen('TextBounds', window, [string mychar]);
                  if norm(3) < responseareawidth
                      % yes, it WILL fit
                      string=[string mychar]; % add new letter to the string
