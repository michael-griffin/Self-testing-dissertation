...
waittime = .3; %blank screen duration for the final practice phase + test phase
...
%Col 3 is JOLs
%Col 4 is jolRT
%Col 5 is blank, used to be restudy/retrieval practice decision.
%Col 6 is what they'll actually do
%Col 7 is JOL rank (higher = better learned)
%Col 8 is JOL percentile (higher = better learned), It's Col 7/List Length
...
studylists = cell(nlists,1);
for m = 1:nlists
    studylist = cell(listlength, 12);
    studylist(:,1:2) = wordpairs(randperm(length(wordpairs)),:); %previous looped through several lists, used start:finish,
    studylists{m,1} = studylist;
end
...
if session == 1
    ...
    instructions = ['Welcome to Day 2 of the experiment. Today, you''ll be tested on the word pairs you learned ' ...
        ...
        'remember the right word, you can enter <b>SKIP</b> to move onto the next word pair.']; %incidental learning gave subjects a skip option, want to do that here?
...
if session == 1
    ...
    for m = 1:nlists
        ...
        studylist = studylists{m,1};
        if m > 1 && ~noinstructions
            Screen('TextSize', window, instructSize);
            instructions = ['You will now have another opportunity to study each word pair. ' ...
                'As before, you should estimate how likely you are to remember the word pair for tomorrow''s test.'];
        ...
        %%%%%%%%%%%
        if m == nlists
            %Update studylist with what they'll be doing during practice.
            ...
            restudylist = restudylist(neworder,:);
        end
    ...
    if ~noinstructions
        Screen('TextSize', window, instructSize);
        instructions = ['For the next part of this experiment ' ... %['The practice phase is about to begin. ' ...
            ...
            'remember the right word, you can enter <b>SKIP</b> to move onto the next word pair.'];
        WriteLine(window, instructions, wordcolor, 50, 50, 100, 2);
        Screen('Flip', window, [], 1);
        WaitSecs(1);
        WriteCentered(window, presstocontinue, xPos, ySize-100, wordcolor);
