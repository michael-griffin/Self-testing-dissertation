...
allstudy = data.frame(matrix(data = NA, nrow = 0, ncol = 13))
allrestudy = data.frame(matrix(data = NA, nrow = 0, ncol=17))
alltest = data.frame(matrix(data = NA, nrow = 0, ncol = 21))
...
#for (i in 23){
  ...
  datstudy = data.frame(matrix(data = NA, nrow = dim(rawstudy)[1], ncol = 15))
  datrestudy = data.frame(matrix(data = NA, nrow = dim(rawrestudy)[1], ncol=19))
  dattest = data.frame(matrix(data = NA, nrow = dim(rawtest)[1], ncol = 23))
  
  #Exp6: now have 'points' and 'value' in between word2 and trial.
  #Exp5: now have choice + honor/dishonor introduced. 
  cnames = c("subn", "word1", "word2", "points", "value", "trial", "list", "jol", "jolrt", "jolrank", "jolrankp",
  ...
  holding = datstudy[start:finish,]
  holding = holding[order(holding$jol),]
  holding$jolrank = jolrank
  holding$jolrankp = jolrankp
  holding = holding[order(holding$trial),]
  datstudy[start:finish,] = holding
  ...
  for (n in 1:dim(rawstudy)[1]){
    ...
    practice[n] = rawrestudy$praccond[index[1]]
  ...
  for (n in 1:dim(rawrestudy)[1]){
    ...
    index = index[length(index)] #grab last index: where delayed JOLs were entered
  ...
  for (n in 1:dim(dattest)[1]){
    ...
    dattest[n,1:dim(datrestudy)[2]] = datrestudy[index,]
    #Previous experiments sometimes had multiple restudy sessions, so would need
    #to skip copying over restudy info. Not a problem here.
